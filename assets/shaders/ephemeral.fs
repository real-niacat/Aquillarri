...
vec4 effect( vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords )
{
    ...
    if (ephemeral.y > ephemeral.y*2) {
        uv.x = 1;
    }
    // float t = lerp(lowest(tex.rgb), 0.5, 0.25)
    float sub = 0.1;
    float p = 2.0;
    tex.a -= sub;
    tex.a = pow(tex.a, p);
    tex.a += sub;
    tex.a -= ((tex.r+tex.g+tex.b)/3.0)*0.2;
    float xd = min(dist(uv.x, 0.0), dist(uv.x, 1.0));
    float yd = min(dist(uv.y, 0.0), dist(uv.y, 1.0));
    float gd = min(xd, yd);
    tex.a -= max(1.0-(gd*10.0), 0);
