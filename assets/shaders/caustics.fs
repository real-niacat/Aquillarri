...
extern PRECISION vec2 caustics;
...
float rsin(float x, float lower, float upper) {
    return lower+(psin(x)*(upper-lower));
}

float noise_at_pos(vec2 pos) {
    return noise(pos*3.0)*2.0*rsin(caustics.y, 0.4, 1.0);
}
...
{
    if (caustics.y > caustics.y*2.0) {
        screen_coords.x *= 2.0;
    }
    vec4 tex = Texel(texture, texture_coords);
	vec2 uv = (((texture_coords)*(image_details)) - texture_details.xy*texture_details.ba)/texture_details.ba;
    vec2 muv = uv + (screen_coords/250.0);
    ...
    for (float i = -count; i <= count; i++) {
        v += noise_at_pos(vec2(muv.x+i, muv.y));
        iters++;
    }
    for (float i = -count; i <= count; i++) {
        v += noise_at_pos(vec2(muv.x, muv.yy));
        iters++;
    }
    v /= sqrt(iters);
    ...
    float width = 5.0;
    v = clamp(v, 0.0, 1.5);
    float hueshift = fract(clamp(v/width, 0.0, 1.0));
    vec3 blue_col = vec3(0.26, 0.36, 0.89);
    tex.rgb += (v*blue_col);
    tex.rgb *= 1-(-min(v, 0.0));
