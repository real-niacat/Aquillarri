...
float pospow(float base, float exponent) {
    return pow(abs(base), exponent) * sign(base);
}

float loop_add(float base, float add) {
    return floor(base)+mod(base+add,1);
}

vec2 loop_add(vec2 base, vec2 add) {
