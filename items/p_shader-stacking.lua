...
-- aquill.add_screen_shader({ key = "aqu_flashlight", send = {
--     { key = "iTime", func = function() return G.TIMERS.REAL end },
--     { key = "screen", func = function() return {love.graphics.getWidth(), love.graphics.getHeight()} end }
-- } })


-- this is REQUIRED
-- due to some jank with shader-stacking, the CRT will not get drawn like it should. 
-- as such, the first screen shader to be applied is the CRT itself
aquill.add_screen_shader({
    ...
    send = {
        ...
        {
            key = "mouse_screen_pos",
            func = function()
                return G.video_control and { love.graphics.getWidth() / 2, love.graphics.getHeight() / 2 } or
